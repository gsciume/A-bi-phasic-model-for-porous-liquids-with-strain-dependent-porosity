...
A collection of **Finite Element Method (FEM)** simulations developed using **FEniCSx (0.9.0)** to model problems involving **porous media mechanics** for **porous liquid** domains. The project is structured into two main simulation cases: a channel flow problem and a transient consolidation problem.

If you use this work, please cite *G. Sciumè and T. Lavigne, A bi-phasic model for porous liquids with strain-dependent porosity, Journal of XXX, 2025*.

> To install the environment (FEniCSx 0.9.0), please refer to (https://github.com/Th0masLavigne/FEniCSx_GMSH_tutorials.git) and (https://th0maslavigne.github.io/FEniCSx_GMSH_tutorials/README.html).
...
.
├── Channel/
│       ├── Croix_porous_liquid.py
│       └── service_files_channel/
└── Consolidation/
        ├── Porous_Liquid.py
        └── service_files_consolidation/
````
...
The simulations rely on the **FEniCS Project** ecosystem. The following dependencies are required:
  ...
  * **Python 3.x**
  * **FEniCSx v0.9.0** (The core Finite Element library)
